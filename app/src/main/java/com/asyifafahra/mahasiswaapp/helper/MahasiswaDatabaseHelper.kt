package com.asyifafahra.mahasiswaapp.helper
...
class  MahasiswaDatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun insertData(data: ModelMahasiswa) {
    ...
    fun getAllData() : List<ModelMahasiswa> {
        val dataList = mutableListOf<ModelMahasiswa>()
        ...
        while (cursor.moveToNext()){
            ...
            val data = ModelMahasiswa(id, nama, nim, jurusan)
            dataList.add(data)
        }
        cursor.close()
        db.close()
    ...
    fun updateData(data : ModelMahasiswa){
        val db = writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_NAMA,data.namaMahasiswa)
            put(COLUMN_NIM,data.nim)
    ...
    fun getDataById(data : Int) : ModelMahasiswa{
        val db = readableDatabase
